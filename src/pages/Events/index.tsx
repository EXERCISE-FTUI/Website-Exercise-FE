...
import "./../../index.css";
import eventsData from "../../assets/events/events.resource.json";
import { useImages } from '../../assets/imageLoader';
import EventsList from "../../assets/events";

// Define the type of box data
interface BoxData {
  id: number;
  image: string;
  title: string;
...
function EventsPage() {
  ...
  const totalBoxes = eventsData.length;
  const totalPages = Math.ceil(totalBoxes / boxesPerPage);

  // Function to render individual boxes
  const renderBox = (boxData: BoxData) => (
    <div
      key={boxData.id}
      className="p-8 md:w-[1175px] md:h-[400px] bg-[#527182] font-inter leading-none rounded-[30px] flex flex-col md:flex-row md:gap-10"
    >
      {boxData.id % 2 === 0 ? (
        <>
          <div className="w-full md:w-1/2 p-3 order-1 md:order-2">
            <img
              src={images[boxData.image] || ''}
              alt={boxData.title}
              className="w-full h-auto md:h-full object-cover rounded-[30px]"
            />
          </div>
          <div className="w-full md:w-1/2 p-3 order-2 md:order-1">
            <h3 className="font-poppins font-bold text-[20px] md:text-[30px] leading-[45px] text-left ">
              {boxData.title}
            </h3>
            <CommonBoxContent boxData={boxData} />
          </div>
        </>
      ) : (
        <>
          ...
          <div className="w-full md:w-1/2 p-3 order-1 md:order-1">
            <img
              src={images[boxData.image] || ''}
              alt={boxData.title}
              className="w-full h-auto md:h-full object-cover rounded-[30px]"
  ...
  const CommonBoxContent = ({ boxData }: { boxData: BoxData }) => (
    <>
      <p className="font-poppins font-medium text-[18px] md:text-[25px] leading-[37.5px] text-left mt-[-10px] flex items-center">
        <svg
      ...
      <p className="font-poppins font-normal italic text-[13px] md:text-[20px] leading-[30px] mt-[10px] text-justify">
        {boxData.description}
      </p>
    </>
  );
  ...
  return (
    <>
      {/* TITLE */}
      <section className="bg-gradient-to-t from-[#132e3b] to-[#1b4358]">
        <div className="text-white text-center font-poppins py-3 px-8 my-0 mx-auto pt-[40px] pb-[40px] md:w-[642px]">
          <h2 className="md:text-[30px] md:pb-3 md:pt-[100px] text-sm  font-bold">Explore Our Events</h2>
      ...
      {/* Triangles, wasted 34 hours g*/}
      <section className="justify-center grid grid-cols-3 gap-0 w-full">
        <div className="relative overflow-hidden">
         <div className=" top-0 left-0 w-0 h-0 border-l-[100px] border-t-[50px] border-transparent border-t-gray-800 sm:border-l-[150px] md:border-l-[100px] lg:border-l-[400px] lg:border-t-[166px]"></div>
        </div>
        <div className="relative overflow-hidden">
            <div className=" top-0 left-0 w-0 h-0 border-l-[100px] border-t-[50px] border-transparent border-t-gray-800 sm:border-l-[150px] md:border-l-[200px] lg:border-l-[400px] lg:border-t-[166px]"></div>
        </div>
        <div className="relative overflow-hidden">
            <div className=" top-0 left-0 w-0 h-0 border-l-[100px] border-t-[50px] border-transparent border-t-gray-800 sm:border-l-[150px] md:border-l-[200px] lg:border-l-[400px] lg:border-t-[166px]"></div>
        </div>
      ...
      <section className="font-poppins text-white bg-gradient-to-t via-exer-charcoal from-[#0B1317] to-[#0B1317] w-full min-h-[70vh] px-[145px] gap-[105px] flex justify-center items-center flex-col relative z-0">
        {<div className="pt-[50px] flex flex-col columns-3 gap-12 font-inter leading-none">
          {eventsData.slice((currentPage - 1) * boxesPerPage, currentPage * boxesPerPage).map(box => renderBox(box))}
        </div>}
        {/* END OF BOXES */}
